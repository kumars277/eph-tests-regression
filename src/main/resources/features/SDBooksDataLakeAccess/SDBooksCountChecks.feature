...
#  Created by Dinesh on 08/09/2020
  ...
  Scenario Outline: Verify Data count for SD transform_previous_history tables are transferred from transformed_previous tables
    Given We know the total count of Current SD data from <SourceTableName>
    Then Get the count of SD transform_file <TargettableName>
    And Check count of SD current table <SourceTableName> and SD tranform_file <TargettableName> are identical

    Examples:
      |SourceTableName                      |TargettableName                                 |
      |sdbooks_transform_current_urls         |sdbooks_transform_file_history_urls_part   |

    @SD
     Scenario Outline: Verify Data count for SDBooks delta_current tables are transferred from Current and Previous tables
      Given Get the difference of total count between current and previous time stamps of transform_file of SDbooks data <TargetTable>
      Then We know the delta current count for Sdbooks tables <TargetTable>
      And Compare SDbooks delta count of <SourceTable> with <TargetTable> are identical
      Examples:
        |SourceTable                                          |TargetTable                            |
        |sdbooks_transform_file_history_urls_part             |sdbooks_delta_current_urls |
