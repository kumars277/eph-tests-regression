Feature:Validate data for JM tables between JM ETL Core tables and JM Staging
    ...
    Scenario Outline: Verify that all JM data is transferred from Staging to JM ETL Core tables
        ...
        Examples:
            ...
            | 500               |etl_accountable_product_dq_v          |
            | 500               |etl_wwork_dq_v                        |
            | 500               |etl_work_identifier_dq_v              |
            | 500               |etl_work_legal_owner_dq_V             |
            | 500               |etl_work_subject_area_dq_v            |
            | 500               |etl_work_person_role_dq_v             |
            | 500               |etl_work_business_model_dq_v          |
            | 500               |etl_work_access_model_dq_v            |
            | 500               |etl_manifestation_dq_v                |
            | 500               |etl_manifestation_updates1_v          |
            | 500               |etl_manifestation_identifier_dq_v     |
            | 500               |etl_product_part1_v                   |
            | 500               |etl_product_inserts_v                 |
            | 500               |etl_product_updates_v                 |
            | 500               |etl_product_dq_v                      |
           # #| 500               |etl_product_person_role_dq_v          | Removed this view as part of JM Flip Change EPHD-3949
            | 500               |sd_subject_areas_v                    |
