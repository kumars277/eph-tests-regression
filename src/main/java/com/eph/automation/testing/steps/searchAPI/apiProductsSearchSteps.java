...
public class apiProductsSearchSteps {
  ...
  public void getRandomProductIdFromPackage() {
    sql =
        String.format(
            APIDataSQL.SELECT_RANDOM_PRODUCT_FROM_PACKAGE, Joiner.on("','").join(packageIds));
    Log.info(sql);
  ...
  */
  ...
  public void compareProductsRetrievedByIsInPackagesOptionWithDB()
    ...
    for (String packageId : packageIds) {
      ...
      returnedProducts.verifyAPIReturnedProductsCount(getNumberOfPackageComponents());
    }
  }
  ...
  private int getNumberOfPackageComponents() {
    sql =
        String.format(
            APIDataSQL.EPH_GD_PRODUCT_COUNT_BY_PACKAGE_EXTRACT, Joiner.on("','").join(packageIds));
    Log.info(sql);
    ...
    for (String id : ids) {
      ...
      Log.info(
          "\n number of packages in API : "
              ...
              + id);
      returnedProducts.verifyProductsAreReturned();
      returnedProducts.verifyAPIReturnedProductsCount(getNumberOfHasComponents());
    }
  }
  ...
  private int getNumberOfHasComponents() {
    sql =
        String.format(
            APIDataSQL.EPH_GD_PACKAGE_COUNT_BY_PRODUCT_EXTRACT, Joiner.on("','").join(ids));
    Log.info(sql);
    List<Map<String, Object>> getCount = DBManager.getDBResultMap(sql, Constants.EPH_URL);
    int count = ((Long) getCount.get(0).get("count")).intValue();
  ...
  public void compareProductsRetrievedByaccountableProductWithDB(String accounableProductType)
      throws AzureOauthTokenFetchingException {
    ...
    for (ProductDataObject productDataObject : productDataObjects) {
      ...
      returnedProducts.verifyAPIReturnedProductsCount(
          getNumberOfProductByAccountableProducts(accountableProductSegmentCode));
    }
  }
  ...
  String getSegmentCode(String accountableProduct) {
    if (accountableProduct != null) {
      ...
      List<Map<String, Object>> list_accountableProduct =
  ...
  private int getNumberOfProductByAccountableProducts(String segmentCode) {
    ...
    List<Map<String, Object>> countByAccountableProduct =
        DBManager.getDBResultMap(sql, Constants.EPH_URL);
  ...
  private int getProductCountByProductStatus(
      String searchTerm, String productStatus) { // created by Nishant @ 5th Dec 2019
    sql =
        APIDataSQL.SELECT_PRODUCTCOUNT_BY_PRODUCTSTATUS
            .replace("param1", searchTerm)
            .replace("param2", productStatus);
  ...
  private int getProductCountByProductType(
      String searchTerm, String productType) { // created by Nishant @ 9th Dec 2019
  ...
  private int getProductCountByWorkType(
      String searchTerm, String workType) { // created by Nishant @ 9th Dec 2019
    sql =
        APIDataSQL.SELECT_PRODUCTCOUNT_BY_WORKTYPE
            .replaceAll("param1", searchTerm)
            .replace("param2", workType);
  ...
  private int getProductCountByManifestationType(
      String searchTerm, String manifestationType) { // created by Nishant @ 9th Dec 2019
    sql =
        String.format(
            APIDataSQL.SELECT_PRODUCTCOUNT_BY_MANIFESTATIONTYPE, searchTerm, manifestationType);
    List<Map<String, Object>> countByManifestationType =
        DBManager.getDBResultMap(sql, Constants.EPH_URL);
  ...
  private int getProductCountByPMCCode(
      String searchTerm, String pmcCode) { // created by Nishant @ 9th Dec 2019
  ...
  private int getProductCountByPMGCode(
      String searchTerm, String pmgCode) { // created by Nishant @ 20th Dec 2019
        ...
        case "PRODUCT_PRODUCT_TITLE":
          ...
          returnedProducts =
              searchForProductsByTitleResult(
                  productDataObject.getPRODUCT_NAME() + "&from=" + from + "&size=" + size);
          ...
          while (!returnedProducts.verifyProductWithIdIsReturnedOnly(
              ...
              && from + size < returnedProducts.getTotalMatchCount()) {
            from += size;
      ...
      for (ProductDataObject productDataObject : productDataObjects) {
        ...
        switch (searchOption) {
          ...
          case "PRODUCT_IDENTIFIER":
            ...
            returnedProducts =
                //   searchForProductsBySearchResult(productIdentifiers.get(0).getIDENTIFIER());
                searchForProductsBySearchResult("123456789");
            break;
            ...
            returnedProducts =
                searchForProductsBySearchResult(
                    dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_TITLE()
                        ...
                        + size);
            Log.info(
                "Total product found for workTitle search... - "
                    + returnedProducts.getTotalMatchCount());
            while (!returnedProducts.verifyProductWithIdIsReturnedOnly(
                    productDataObjects.get(0).getPRODUCT_ID())
                && from + size < returnedProducts.getTotalMatchCount()) {
          ...
          case "PRODUCT_WORK_IDENTIFIER":
            getWorkIdentifiers(productDataObjects.get(0).getF_PRODUCT_WORK());
            returnedProducts =
                searchForProductsBySearchResult(workIdentifiers.get(0).getIDENTIFIER());
            break;
            ...
            returnedProducts =
                searchForProductsBySearchResult(
                    manifestationDataObjects.get(0).getMANIFESTATION_KEY_TITLE()
                        ...
                        + size);
            Log.info(
                "Total product found for manifestationTitle search... - "
                    + returnedProducts.getTotalMatchCount());
            while (!returnedProducts.verifyProductWithIdIsReturnedOnly(
                    productDataObjects.get(0).getPRODUCT_ID())
                && from + size < returnedProducts.getTotalMatchCount()) {
            ...
            returnedProducts =
                searchForProductsBySearchResult(
                    dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_TITLE()
                        ...
                        + size);
            Log.info(
                "Total product found for manifestationWorkTitle search... - "
                    + returnedProducts.getTotalMatchCount());
            while (!returnedProducts.verifyProductWithIdIsReturnedOnly(
                    productDataObjects.get(0).getPRODUCT_ID())
                && from + size < returnedProducts.getTotalMatchCount()) {
            ...
            returnedProducts =
                searchForProductsBySearchResult(
                    personDataObjectsFromEPHGD.get(0).getPERSON_FIRST_NAME()
                        + " "
                        + personDataObjectsFromEPHGD.get(0).getPERSON_FAMILY_NAME()
                        + "&from="
                        + from
                        + "&size="
                        + size);
            Log.info(
                "Total product found for product person full name... - "
                    + returnedProducts.getTotalMatchCount());
            while (!returnedProducts.verifyProductWithIdIsReturnedOnly(
                    productDataObjects.get(0).getPRODUCT_ID())
                && from + size < returnedProducts.getTotalMatchCount()) {
            ...
            returnedProducts =
                searchForProductsBySearchResult(
                    personDataObjectsFromEPHGD.get(0).getPERSON_FIRST_NAME()
                        ...
                        + size);
            Log.info(
                "Total product found for product work person full name... - "
                    + returnedProducts.getTotalMatchCount());
            while (!returnedProducts.verifyProductWithIdIsReturnedOnly(
                    productDataObjects.get(0).getPRODUCT_ID())
                && from + size < returnedProducts.getTotalMatchCount()) {
            ...
            returnedProducts =
                searchForProductsBySearchResult(
                    personDataObjectsFromEPHGD.get(0).getPERSON_FIRST_NAME()
                        ...
                        + size);
            Log.info(
                "Total product found for product manifestation work person full name... - "
                    + returnedProducts.getTotalMatchCount());
            while (!returnedProducts.verifyProductWithIdIsReturnedOnly(
                    productDataObjects.get(0).getPRODUCT_ID())
                && from + size < returnedProducts.getTotalMatchCount()) {
        ...
        if (searchOption.equalsIgnoreCase("PRODUCT_IDENTIFIER")) {
          returnedProducts.verifyNoProductReturned();
        } else {
          returnedProducts.verifyProductsAreReturned();
          returnedProducts.verifyProductWithIdIsReturned(productDataObject.getPRODUCT_ID());
  ...
  public void compareProductsRetrievdByPersonWithDB() throws AzureOauthTokenFetchingException {
    ...
    for (String id : ids) {
      ...
      returnedProducts.verifyProductsAreReturned();
      returnedProducts.verifyAPIReturnedProductsCount(getNumberOfProductsByPersonIDs(id));
    }
  }
  ...
  public void compareProductSearchByPMGResultsWithDB() throws AzureOauthTokenFetchingException {
    ...
    for (ProductDataObject productDataObject : productDataObjects) {
      ...
      returnedProducts.verifyAPIReturnedProductsCount(getProductsCountByPMG(pmgCode));
    }
  }
  ...
  private int getProductsCountByManifestations() {
    sql =
        String.format(
            APIDataSQL.SELECT_COUNT_PRODUCTS_BY_MANIFESTATIONS,
  ...
  private int getProductsCountByPMG(String pmgCode) {
    return getProductCountByPMGCode(pmgCode);
  }
    ...
    try {
      ...
      switch (paramKey) {
        case "productStatus":
          ...
          productCount_DB =
              getProductCountByProductStatus(
                  defaultSearch, productDataObjects.get(0).getF_STATUS());
          returnedProducts =
        ...
        case "productType":
          ...
          productCount_DB =
              getProductCountByProductType(defaultSearch, productDataObjects.get(0).getF_TYPE());
          break;
        case "workType":
          ...
          productCount_DB =
              getProductCountByWorkType(
                  defaultSearch, dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_TYPE());
          break;
        case "manifestationType":
          ...
          productCount_DB =
              getProductCountByManifestationType(
                  defaultSearch, manifestationDataObjects.get(0).getF_TYPE());
          break;
        case "pmcCode":
          getWorkByManifestationID(productDataObjects.get(0).getF_PRODUCT_MANIFESTATION_TYP());
          DataQualityContext.breadcrumbMessage +=
          ...
          productCount_DB =
              getProductCountByPMCCode(
                  searchTerm, dataQualityContext.workDataObjectsFromEPHGD.get(0).getPMC());
          break;
        case "pmgCode":
          getWorkByManifestationID(productDataObjects.get(0).getF_PRODUCT_MANIFESTATION_TYP());
          String pmgCode =
  ...
  public void theProductTitleAreRetrievedAndCotainsSearchKey(String paramKey) throws Throwable {
    // created by Nishant @ 04 Jun 2021 to debug issue while API and DB count mismatch for search
    // result
    ...
    switch (paramKey) {
      case "productStatus":
        DataQualityContext.breadcrumbMessage += "->" + productDataObjects.get(0).getF_STATUS();
        productCount_DB =
            getProductCountByProductStatus(defaultSearch, productDataObjects.get(0).getF_STATUS());

        returnedProducts =
            searchForProductByParam(
                defaultSearch + "&from=" + from + "&size=" + size,
                paramKey,
