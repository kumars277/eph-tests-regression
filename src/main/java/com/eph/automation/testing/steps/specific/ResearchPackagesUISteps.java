...
public class ResearchPackagesUISteps {
    ...
    public ResearchPackagesUISteps researchPackagesUISteps;
    ...
    public void chckDJCicon(){
         int totSize = tasks.getSize("XPATH",ResearchPackagesConstants.SELECT_DJC);
         Log.info("DJC Collection Size: "+ totSize);
        Assert.assertEquals("DJC icon is missing",1,totSize);
    }

    @Then("^Check any DJC Collections are present$")
    public void listOfCollectionsDJC(){
        if(tasks.getSize("XPATH",ResearchPackagesConstants.SELECT_DJC)>0){
            tasks.click("XPATH",ResearchPackagesConstants.SELECT_DJC);
            Log.info("List of Collections Available under DJC: "+tasks.getTextofAllElements("XPATH",ResearchPackagesConstants.LIST_OF_COLLECTIONS));
            Log.info("Total DJC Collections Available: "+tasks.getSize("XPATH",ResearchPackagesConstants.LIST_OF_COLLECTIONS));
            Assert.assertNotEquals("No Collections Available under DJC",0,tasks.getSize("XPATH",ResearchPackagesConstants.LIST_OF_COLLECTIONS));

        }
    }

    @Given("^User in the Freedom Collection Page$")
    public void enterFCpage() throws Throwable {
        if(tasks.getSize("XPATH",ResearchPackagesConstants.FREESOM_COLLECTION)>0) {
                    tasks.click("XPATH", ResearchPackagesConstants.FREESOM_COLLECTION);
                    if (tasks.getTextofElement("XPATH", ResearchPackagesConstants.COLLECTION_TITLE).equalsIgnoreCase("Freedom Collection")) {
                        Log.info("User in the Freedom Collection Page");
                    }
                }
        }

    @Then ("^Search Journal by issn (.*) in Add Journal Box$")
    public void searchJournalByIssn(String Issn){
        try {
            researchPackagesTasks.addJournals();
            researchPackagesTasks.validateSearchAddJournalByIssn(Issn);
        }catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    @Then ("^Search Journal by title (.*) in Add Journal Box$")
    public void searchJournalByTitle(String title){
        try {
            researchPackagesTasks.addJournals();
            researchPackagesTasks.validateSearchAddJournalByIssn(title);
        }catch (InterruptedException e) {
            e.printStackTrace();
        }
    }

    @And ("^Verify (.*) and (.*) and (.*) for (.*) from search result$")
    public void validateSearchResultInAddJournal(String title,String jNo,String pubDir,String issn) throws InterruptedException {
        Assert.assertTrue(jNo+" journal number not matching/missing in the search result for "+issn,tasks.verifyElementisDisplayed("XPATH","//td[contains(text(),'"+jNo+"')]"));
        Assert.assertTrue(title+" title not matching/missing in the search result for "+issn,tasks.verifyElementisDisplayed("XPATH","//td[contains(text(),'"+title+"')]"));;
      //  Assert.assertTrue(issn+" issn not matching/missing in the search result",tasks.verifyElementisDisplayed("XPATH","//td[contains(text(),'"+issn+"')]"));
        Assert.assertTrue(pubDir+" pub director not matching/missing in search result for "+issn,tasks.verifyElementisDisplayed("XPATH","//td[contains(text(),'"+pubDir+"')]"));
    }

    @Then("^Close the Add Journal box$")
        public void closeAddJournalBox() throws InterruptedException {
                if(tasks.verifyElementisDisplayed("XPATH", ResearchPackagesConstants.ADD_JOURNAL_CLOSE)) {
                    tasks.click("XPATH", ResearchPackagesConstants.ADD_JOURNAL_CLOSE);
                }
            }
