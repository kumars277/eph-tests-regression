      ...
      for (int wp = 0; wp < workParent.length; wp++) {
        for (int wp2 = 0; wp2 < workParent.length; wp2++) {
          if (!list_workParent
              ...
              .equalsIgnoreCase(
                  dataQualityContext
                      .workRelationshipParentDataObjectsFromEPGD
                      .get(wp2)
                      .getF_PARENT())) ;
          else {
            printLog("verifying workParent " + list_workParent.get(wp).id);
            Assert.assertEquals(
                workId + " - parent relationshipType",
                list_workParent.get(wp).type.get("code"),
                dataQualityContext
                    .workRelationshipParentDataObjectsFromEPGD
                    .get(wp2)
                    .getF_RELATIONSHIP_TYPE());
            printLog("relationshipType");

            Assert.assertEquals(
                workId + " - parent effectiveStartDate",
                list_workParent.get(wp).effectiveStartDate,
                dataQualityContext
                    .workRelationshipParentDataObjectsFromEPGD
                    .get(wp2)
                    .getEFFECTIVE_START_DATE());
            printLog("effectiveStartDate");

            getWorksDataFromEPHGD(list_workParent.get(wp).id);
            // Log.info("comparing\n-title\n-work Type code\n-work status code");
            Assert.assertEquals(
                workId + " - parent work Title",
                list_workParent.get(wp).workSummary.title,
                dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_TITLE());
            printLog("work Title");

            Assert.assertEquals(
                workId + " - parent work Type",
                list_workParent.get(wp).workSummary.type.get("code"),
                dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_TYPE());
            printLog("work Type");

            Assert.assertEquals(
                workId + " -parent work Status",
                list_workParent.get(wp).workSummary.status.get("code"),
                dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_STATUS());
      ...
      for (int wc = 0; wc < workChild.length; wc++) {
        for (int wc2 = 0; wc2 < workChild.length; wc2++) {
          if (!(list_workChild
                  ...
                  .equalsIgnoreCase(
                      dataQualityContext
                          .workRelationshipChildDataObjectsFromEPGD
                          .get(wc2)
                          .getF_CHILD())
              & list_workChild
                  .get(wc)
            ...
            //   Assert.assertEquals(workId+ " - child
            // relationshipType",list_workChild.get(wc).type.get("code"),
            // dataQualityContext.workRelationshipChildDataObjectsFromEPGD.get(wc2).getF_RELATIONSHIP_TYPE());

            Assert.assertEquals(
                workId + " - child effectiveStartDate",
                list_workChild.get(wc).effectiveStartDate,
                dataQualityContext
                    .workRelationshipChildDataObjectsFromEPGD
                    .get(wc2)
                    .getEFFECTIVE_START_DATE());
            ...
            //     Log.info("comparing \n-title\n-work Type code\n-work status code");
            Assert.assertEquals(
                workId + " - child work Title",
                list_workChild.get(wc).workSummary.title,
                dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_TITLE());
            printLog("work Title");

            Assert.assertEquals(
                workId + " - child work Type",
                list_workChild.get(wc).workSummary.type.get("code"),
                dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_TYPE());
            printLog("work Type");

            Assert.assertEquals(
                workId + " - child work Status",
                list_workChild.get(wc).workSummary.status.get("code"),
                dataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_STATUS());
            printLog("work Status");
            if (list_workChild.get(wc).effectiveEndDate != null) {
              Assert.assertEquals(
                  workId + " - child effectiveStartDate",
                  list_workChild.get(wc).effectiveEndDate,
                  dataQualityContext
                      .workRelationshipChildDataObjectsFromEPGD
                      .get(wc2)
                      .getEFFECTIVE_END_DATE());
