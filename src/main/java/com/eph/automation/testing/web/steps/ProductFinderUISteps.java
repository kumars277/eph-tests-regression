...
public class ProductFinderUISteps {
    ...
    private static List<String> productIdList,manifestIdList;
    private static List<String>productTitleList,manifestTitleList,manifestWorkList;
    ...
    private static List<Map<?, ?>> availableProduct, availableManifestation;
    private static String productId, manifestId, productTitle, manifestTitle, manifestWork;
    private static String  buildProductClickableLink, buildManifestClickableLink;
    @Inject
    ...
    public void verifyUserIsForwardedToWorksPage () {
        assertTrue(productFinderTasks.isUserOnDetailsPage(DataQualityContext.workDataObjectsFromEPHGD.get(0).getWORK_ID(),"work"));
    }
  /*
  ...
  @Given("^Test$")
    ...
    public void getManifestationfromDb() throws Throwable {
        sql = String.format(ProductFinderSQL.SELECT_MANIFESTATIONID_TITLE_FOR_SEARCH);
        Log.info(sql);
        availableManifestation = DBManager.getDBResultMap(sql, Constants.EPH_URL);
        if(availableManifestation.size() > 0){
            manifestIdList       = availableManifestation.stream().map(m -> (String) m.get("MANIFESTATION_ID")).map(String::valueOf).collect(Collectors.toList());
            manifestTitleList    = availableManifestation.stream().map(m -> (String) m.get("MANIFESTATION_TITLE")).map(String::valueOf).collect(Collectors.toList());
            manifestWorkList     =  availableManifestation.stream().map(m -> (String) m.get("MANIFESTATION_WORK_ID")).map(String::valueOf).collect(Collectors.toList());
        }
        manifestId = manifestIdList.toString();
        manifestId =   manifestId.replaceAll("\\[", "").replaceAll("\\]", "");
        manifestTitle = manifestTitleList.toString();
        manifestTitle =   manifestTitle.replaceAll("\\[", "").replaceAll("\\]", "");
        manifestWork = manifestWorkList.toString();
        manifestWork =   manifestWork.replaceAll("\\[", "").replaceAll("\\]", "");
    }

    @And("^Searches for Manifestation by id$")
    public void searchManifestationId() throws Throwable {
        productFinderTasks.searchFor(manifestId);
        tasks.click("XPATH",ProductFinderConstants.manifestationsTab);
        Log.info("Manifestation ID => "+manifestId);
        Thread.sleep(1000);
    }

    @And("^Searches for Manifestation by title$")
    public void searchManifestationTitle() throws Throwable {
        productFinderTasks.searchFor(manifestTitle);
        tasks.click("XPATH",ProductFinderConstants.manifestationsTab);
        Log.info("Manifestation Title => "+manifestTitle);
        Thread.sleep(1000);
    }

    @Then ("^Verify the searched manifestation are displayed in the result$")
    public void verifyManifestDisplayed() throws InterruptedException {
        Log.info("Manifest ID => "+manifestId);
        Log.info("Manifest Title =>"+manifestTitle);
        while (!productFinderTasks.isPageContainingString(manifestId)&&
                productFinderTasks.isPageContainingString(manifestTitle)) {
            productFinderTasks.goToNextPage();
        }
        assertTrue("Product is successfully Displayed by Id",productFinderTasks.isPageContainingString(manifestId));
        assertTrue("Product is successfully Displayed by Title",productFinderTasks.isPageContainingString(manifestTitle));
        buildManifestClickableLink = "//a[@href='/manifestation/"+manifestWork+"/"+ manifestId + "/overview']";
        Log.info(buildManifestClickableLink);
    }

    @And ("^Click the Result and User is forwarded to the manifestation detail page$")
    public void forwadedToManifestDetailPage() throws InterruptedException {
        tasks.click("XPATH",buildManifestClickableLink);
        Thread.sleep(1000);
        String manifestIDWork = manifestWork+"/"+manifestId;
        assertTrue(productFinderTasks.isUserOnDetailsPage(manifestIDWork,"manifestation"));
    }

    @Given("^We get the id and title for product search from DB$")
    public void getProductfromDb() throws Throwable {
        ...
        productTitle = productTitleList.toString();
        productTitle =   productTitle.replaceAll("\\[", "").replaceAll("\\]", "");

    }

    @And("^Searches for Product by id$")
    public void searchProductId() throws Throwable {
        productFinderTasks.searchFor(productId);
        tasks.click("XPATH",ProductFinderConstants.productsPackagesTab);
        Log.info("Product ID => "+productId);
        Thread.sleep(1000);
    }

    @And("^Searches for Product by title$")
    public void searchProductTitle() throws Throwable {
        productFinderTasks.searchFor(productTitle);
        tasks.click("XPATH",ProductFinderConstants.productsPackagesTab);
    ...
    public void verifyProductDisplayed() throws InterruptedException {
        Log.info("Product ID => "+productId);
        Log.info("Product Title =>"+productTitle);
            while (!productFinderTasks.isPageContainingString(productId)&&
                    productFinderTasks.isPageContainingString(productTitle)) {
                productFinderTasks.goToNextPage();
            }
            assertTrue("Product is successfully Displayed by Id",productFinderTasks.isPageContainingString(productId));
            assertTrue("Product is successfully Displayed by Title",productFinderTasks.isPageContainingString(productTitle));
            buildProductClickableLink = "//a[@href='/product/" + productId + "/overview']";
            Log.info(buildProductClickableLink);
    }

    @And ("^Click the Result and User is forwarded to the product detail page$")
    public void forwadedToProductDetailPage() throws InterruptedException {
        tasks.click("XPATH",buildProductClickableLink);
        Thread.sleep(1000);
        assertTrue(productFinderTasks.isUserOnDetailsPage(productId,"product"));
    }
        ...
        try{
            if(workTypesCodeAvailable.size()>0) {
                for (String booksworkId : booksworkIdList) {
                    ...
                    tasks.click("XPATH", buildWorkIdLocator);
                    Thread.sleep(1000);
                    assertTrue(productFinderTasks.isUserOnDetailsPage(booksworkId,"work"));
                    if (chooseWorkType.equalsIgnoreCase("Book")) {
