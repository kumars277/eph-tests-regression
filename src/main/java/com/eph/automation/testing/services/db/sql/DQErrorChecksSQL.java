...
public class DQErrorChecksSQL {
    ...
    public static String GET_FAILED_DATA = "select pmx_source_reference as pmx_id from "+GetEPHDBUser.getDBUser()+".stg_10_pmx_wwork_dq where dq_err='Y'" +
    ...
    public static String GET_All_FAILED_DATA = "select pmx_source_reference as pmx_id from "+GetEPHDBUser.getDBUser()+".stg_10_pmx_wwork_dq where dq_err='Y'";

    public static String GET_All_FAILED_Manifestations = "select pmx_source_reference as pmx_id from "+GetEPHDBUser.getDBUser()+".stg_10_pmx_manifestation_dq where dq_err='Y'";
    ...
    public static String GET_EPH_ID = "select eph_id as work_id from "+GetEPHDBUser.getDBUser()+".map_sourceref_2_ephid\n" +
            "where ref_type = 'WORK' and source_ref in ('%s')";

    public static String GET_FAILED_Manifestation_DATA = "select pmx_source_reference as pmx_id from "+GetEPHDBUser.getDBUser()+".stg_10_pmx_manifestation_dq where dq_err='Y'" +
            " ORDER BY RANDOM() limit PARAM1";
    ...
    public static String GET_FAILED_Person_DATA = "select person_source_ref as pmx_id from "+GetEPHDBUser.getDBUser()+".stg_10_pmx_person_dq where person_family_name is null" +
            " ORDER BY RANDOM() limit PARAM1";

    public static String GET_DQ_Status_Person = "select dq_err as dq_err, person_source_ref as pmx_source from "+GetEPHDBUser.getDBUser()+".stg_10_pmx_person_dq where " +
            " person_source_ref in ('%s')";

    public static String GET_Person_EPH_ID = "select id as eph_id from "+GetEPHDBUser.getDBUser()+".map_sourceref_2_numericid where " +
            " source_ref = 'PARAM1'";
}
