...
public class ProductDataSQL {
    ...
    public static String SELECT_RANDOM_PRODUCT_MANIFESTATION_IDS_FOR_JOURNALS = "select \"PRODUCT_MANIFESTATION_ID\" as PRODUCT_MANIFESTATION_ID from ephsit_talend_owner.stg_10_pmx_product stg \n " +
            "left join ephsit_talend_owner.stg_10_pmx_product_dq dq on dq.product_short_name = stg.\"PRODUCT_SHORT_NAME\"\n" +
            "left join ephsit_talend_owner.stg_10_pmx_wwork_dq w on dq.f_work_source_ref = w.pmx_source_reference \n" +
            "left join ephsit_talend_owner.stg_10_pmx_manifestation_dq m on dq.f_manifestartion_source_ref = m.pmx_source_reference \n" +
            "left join semarchy_eph_mdm.sa_product sa on dq.pmx_source_reference = sa.pmx_source_reference\n" +
            " where \"SUBSCRIPTION\" = 'Y' \n" +
            "and dq.dq_err != 'Y' and w.dq_err != 'Y' and m.dq_err != 'Y' \n" +
