...
public class crossRefDLSQL {
    ...
    public static String GET_IDENTIFIER =
            "select identifier as identifier from(\n" +
                    "SELECT DISTINCT *\n" +
                    "FROM\n" +
                    "  (\n" +
                    "   WITH\n" +
                    ...
                    "   INNER JOIN product_type pt ON (p.product_id = pt.product_id))\n" +
                    "   WHERE (p.external_reference IS NOT NULL)\n" +
                    ") as x )xt order by rand() limit %s";

    public static String GET_IDENTIFIER_RECS =
            "select * from(\n" +
                    "SELECT DISTINCT *\n" +
                    "FROM\n" +
                    "  (\n" +
                    ...
                    "   INNER JOIN product_type pt ON (p.product_id = pt.product_id))\n" +
                    "   WHERE (p.external_reference IS NOT NULL)\n" +
                    ") as x )xt where identifier in ('%s') order by identifier desc,identifier_type desc,record_level desc, epr desc";

    public static String GET_CROSS_IDENTIFIER_RECS =
            "select * from "+ GetJMDLDBUser.getCrossRefDb()+".eph_identifier_cross_reference where identifier in ('%s') order by identifier desc,identifier_type desc,record_level desc, epr desc";
