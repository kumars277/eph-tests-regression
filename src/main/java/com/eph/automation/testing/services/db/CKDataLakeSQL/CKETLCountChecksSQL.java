...
//  Delta History Count SQL
    public static String GET_CK_Delta_History = "select count(*) as COUNT from "+GetCKDLDB.getCKDataBase()+".%s where inbound_ts in (select distinct inbound_ts from "+GetCKDLDB.getCKDataBase()+".%s order by inbound_ts desc limit 1)";
