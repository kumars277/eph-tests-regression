...
public class CKReportsDataChecksSQL {
    ...
    public static String  GET_WORKFLOW_TABLEAU_VIEW_IDs =
            "WITH\n" +
                    "  packages AS (\n" +
                    "   SELECT\n" +
                    "     ww.work_id pkg_epr\n" +
                    ...
                    ") \n" +
                    "SELECT distinct pco_epr \"Work_ID\"\n" +
                    "FROM\n" +
                    "  ((packages\n" +
                    "INNER JOIN package_component_relations ON (packages.pkg_epr = package_component_relations.pkgr_owner_epr))\n" +
                    "INNER JOIN package_components ON (package_component_relations.pkgr_component_epr = package_components.pco_epr)) limit %s\n";

    public static String GET_WORKFLOW_CONTROL_P1_VIEW_IDs =
    ...
    public static String GET_WORKFLOW_CONTROL_P2_VIEW_IDs =
            "select workflow_id from(\n" +
                    "    SELECT\n" +
                    "            workflow_id\n" +
                    "            , key_snapshot\n" +
    ...
    public static String GET_WORKFLOW_TABLEAU_PACKAGE_WORKS_VIEW_IDs =
            "select work_id from (\n" +
                    "   SELECT\n" +
                    "     ctrl.workflow_type\n" +
    ...
    public static String GET_TRANSACTION_WORKFLOW_VIEW_IDs =
            "WITH\n"+
                    "  transactions AS (\n"+
                    "   SELECT\n"+
                    "     tw.workflow_id\n"+
    ...
    public static String GET_WORKFLOW_TABLEAU_VIEW_Data =
            "WITH\n" +
                    "  packages AS (\n" +
                    "   SELECT\n" +
                    "     ww.work_id pkg_epr\n" +
                    ...
                    ") \n" +
                    "SELECT" +
                    "  pkg_title \"Package_Title\"\n" +
                    ", pkg_type \"CK_Package_Type\"\n" +
                    ", pkg_type_name \"CK_Package_Description\"\n" +
                    ", pkg_status_name \"Package_Status\"\n" +
                    ", pkg_status_roll_up \"Package_Status_Roll_Up\"\n" +
                    ", pkg_acronym \"Package_Code\"\n" +
                    ", pkg_hl3_code \"CK_Site_Code\"\n" +
                    ", pkg_hl3_name \"CK_Site_Name\"\n" +
                    ", pkg_hl2_code \"Sub_platform_code\"\n" +
                    ", pkg_hl2_name \"Sub_Platform_Name\"\n" +
                    ", pkgr_owner_epr \"Package_ID\"\n" +
                    ", pkgr_component_epr \"Package_Work_ID\"\n" +
                    ", pkgr_rel_start_date \"Pkg_wk_effective_start_date\"\n" +
                    ", pkgr_rel_end_date \"Pkg_wk_effective_end_date\"\n" +
                    ", pkgr_rel_durable_url \"Durable_URL\"\n" +
                    ", pkgr_rel_durable_url_live_date \"Durable_URL_Live_Date\"\n" +
                    ", pco_epr \"Work_ID\"\n" +
                    ", pco_title \"Work_Title\"\n" +
                    ", pco_issnl \"ISSN_L\"\n" +
                    ", pco_lead_isbn \"Lead_ISBN\"\n" +
                    ", pco_edition_number \"Edition_Number\"\n" +
                    ", pco_copyright_year \"Copyright_Year\"\n" +
                    ", pco_actual_launch_date \"Actual_Launch_Date\"\n" +
                    ", pco_type_roll_up \"Work_Type_Group\"\n" +
                    ", pco_status_name \"Package_Component_Status\"\n" +
                    ", pco_status_roll_up \"Package_Component_Status_Roll_up\"\n" +
                    ", pco_imprint_name \"Imprint\"\n" +
                    ", pco_pmc \"PMC\"\n" +
                    ", pco_pmc_name \"PMC_Description\"\n" +
                    ", pco_pmg \"PMG\"\n" +
                    ", pco_pmg_name \"PMG_Description\"\n" +
                    ", pco_language_name \"Language\"\n" +
                    ", pco_specialty \"Specialty_Name\"\n" +
                    ", pco_author \"Author\"\n" +
                    ", pco_sequence \"Author_Sequence\"\n" +
                    ", pco_editor \"Editor\"\n" +
                    ", pco_editor_seq \"Editor_Seq\"\n" +
                    ", pco_ck_title_start_date \"CK_Title_Start_Date\"\n" +
                    ", pco_ck_item_live_date \"Workflow_Live_Date\"\n" +
                    ", pco_ck_workflow_id \"Workflow_Id\"\n" +
                    ", pco_ck_alternate_title \"CK_Alternate_Title\"\n" +
                    ", pco_ck_search_tier \"Search_Tier\"\n" +
                    ", pco_ck_finance_tier \"Finance_Tier\"\n" +
                    ", pco_journal_backfile_years \"Journal_Backfile_Years\"\n" +
                    ", pco_ck_pdf_suppression \"PDF_Suppression\"\n" +
                    ", pco_video \"Video\"\n" +
                    ", pco_topic_pages \"Topic_Pages\"\n" +
                    ", pco_journal_backfiles_only \"Journal_Backfiles_Only\"\n" +
                    ", pco_aip \"AIP\"\n" +
                    ", pco_prev_work_id \"Previous_Work_ID\"\n" +
                    ", pco_prev_lead_isbn \"Previuos_Lead_ISBN\"\n" +
                    ", pco_next_work_id \"Next_Work_ID\"\n" +
                    ", pco_next_lead_isbn \"Next_Lead_ISBN\"\n" +
                    ", pco_ownership \"Ownership\"\n" +
                    ", pco_firstpubdate \"First_Pub_Date\"\n" +
                    " FROM\n" +
                    "  ((packages\n" +
                    "INNER JOIN package_component_relations ON (packages.pkg_epr = package_component_relations.pkgr_owner_epr))\n" +
                    "INNER JOIN package_components ON (package_component_relations.pkgr_component_epr = package_components.pco_epr)) " +
                    "where pco_epr in ('%s') order by Work_ID ASC, Package_Title ASC, Specialty_Name ASC, CK_Site_Name ASC," +
                    " CK_Title_Start_Date ASC, Author ASC,Editor ASC\n";


    public static String GET_WORKFLOW_CONTROL_P1_VIEW_Data =
                "select * from(\n"+
                    "SELECT\n"+
                    "  allposs.workflow_id workflow_id\n"+
                    ", \"min\"(allposs.snapshot_ts) key_snapshot\n"+
    ...
    public static String GET_WORKFLOW_CONTROL_P2_VIEW_Data =
            "select * from(\n" +
                    "    SELECT\n" +
                    "            workflow_id\n" +
                    "            , key_snapshot\n" +
    ...
    public static String GET_WORKFLOW_TABLEAU_PACKAGE_WORKS_VIEW_Data =
            "select * from (\n" +
                    "   SELECT\n" +
                    "     ctrl.workflow_type\n" +
                    "   , part.workflow_live_date\n" +
                    ...
                    "   GROUP BY ctrl.workflow_type, part.workflow_live_date, part.workflow_id, part.snapshot_ts, part.package_id, part.package_code, part.package_title, part.ck_site_code, part.ck_site_name, part.sub_platform_code, part.sub_platform_name, part.ck_package_type, part.work_id, part.work_type_group, part.work_title, part.ck_alternate_title, part.issn_l, part.lead_isbn, part.edition_number, part.author, part.author_sequence, part.editor, part.editor_seq, part.specialty_name, part.finance_tier, part.search_tier, part.previous_work_id, part.previuos_lead_isbn, part.next_work_id, part.next_lead_isbn, part.ownership, part.first_pub_date\n" +
                    ")where work_id in ('%s') " +
                    "order by workflow_type asc,workflow_live_date asc,workflow_id asc,snapshot_ts asc,package_id asc,package_code asc,package_title asc, ck_site_code asc" +
                    ",ck_site_name asc,sub_platform_code asc,sub_platform_name asc,ck_package_type asc,work_id asc,\n" +
                    "work_type_group asc, work_title asc,ck_alternate_title asc,issn_l asc,lead_isbn asc,\n" +
                    "edition_number asc,author asc,author_sequence asc,editor asc,editor_seq asc,specialty_name asc, finance_tier asc, \n" +
                    "search_tier asc,previous_work_id asc,previuos_lead_isbn asc,next_work_id asc,next_lead_isbn asc,ownership asc,first_pub_date asc";


    public static String GET_TRANSACTION_WORKFLOW_VIEW_Data =
            "WITH\n"+
                    "  transactions AS (\n"+
                    "   SELECT\n"+
                    "     tw.workflow_id\n"+
                    ...
                    "INNER JOIN package_components pkg_comp ON (tn.eph_work_id = pkg_comp.Work_ID))\n"+
                    "INNER JOIN packages pkg ON (pkg_comp.Package_ID = pkg.Package_ID))\n"+
                    ") where Work_ID in ('%s') order by Workflow_ID asc, Workflow_Month asc,Workflow_Year asc,CutOff_Date asc,Workflow_Live_Date asc,Workflow_Type asc,\n"+
                    "Workflow_Status asc,Transaction_Audit_ID asc,Work_ID asc,Action asc,Description asc,Actioned_By asc,Actioned_On asc,Reason asc,Comment asc,IsEmergency_Update asc,Workflow_Code asc," +
                    "Transaction_Field_Change_ID asc,Field asc,from_value asc,To_Value asc,Work_Type_Group asc,ISSN_L asc,Lead_ISBN asc,Package_ID asc,Package_Title asc," +
                    "Package_Code asc,CK_Site_Code asc,CK_Site_Name asc,Sub_platform_code asc,Sub_Platform_Name asc,CK_Package_Type asc";

    // Reports Table data SQL
    public static String GET_WORKFLOW_TABLEAU_TABLE_Data =
            "select * from " + GetCKDLDB.getCKCMMSDataBase() +".ck_workflow_tableau where Work_ID in ('%s') " +
                    "order by Work_ID ASC, Package_Title ASC, Specialty_Name ASC, CK_Site_Name ASC, CK_Title_Start_Date ASC, Author ASC,Editor ASC";

    public static String GET_WORKFLOW_CONTROL_P1_TABLE_Data = "select * from " + GetCKDLDB.getCKCMMSDataBase() + ".ck_workflow_control_p1 where workflow_id in (%s)";

    public static String GET_WORKFLOW_CONTROL_P2_TABLE_Data = "select * from " + GetCKDLDB.getCKCMMSDataBase() + ".ck_workflow_control_p2 where workflow_id in (%s)";

    public static String GET_WORKFLOW_TABLEAU_PACKAGE_WORKS_TABLE_Data = "select * from " + GetCKDLDB.getCKCMMSDataBase() + ".ck_workflow_tableau_package_works " +
            "where work_id in ('%s') " +
            "order by workflow_type asc,workflow_live_date asc,workflow_id asc,snapshot_ts asc,package_id asc,package_code asc,package_title asc, ck_site_code asc" +
            ",ck_site_name asc,sub_platform_code asc,sub_platform_name asc,ck_package_type asc,work_id asc,\n" +
            "work_type_group asc, work_title asc,ck_alternate_title asc,issn_l asc,lead_isbn asc,\n" +
            "edition_number asc,author asc,author_sequence asc,editor asc,editor_seq asc,specialty_name asc, finance_tier asc, \n" +
            "search_tier asc,previous_work_id asc,previuos_lead_isbn asc,next_work_id asc,next_lead_isbn asc,ownership asc,first_pub_date asc";
    public static String GET_TRANSACTION_WORKFLOW_TABLE_Data =
            "select * from " + GetCKDLDB.getCKCMMSDataBase() + ".ck_transaction_workflow where Work_ID in ('%s') order by Workflow_ID asc, Workflow_Month asc,Workflow_Year asc,CutOff_Date asc,Workflow_Live_Date asc,Workflow_Type asc,\n"+
                    "Workflow_Status asc,Transaction_Audit_ID asc,Work_ID asc,Action asc,Description asc,Actioned_By asc,Actioned_On asc,Reason asc,Comment asc,IsEmergency_Update asc,Workflow_Code asc," +
                    "Transaction_Field_Change_ID asc,Field asc,from_value asc,To_Value asc,Work_Type_Group asc,ISSN_L asc,Lead_ISBN asc,Package_ID asc,Package_Title asc," +
                    "Package_Code asc,CK_Site_Code asc,CK_Site_Name asc,Sub_platform_code asc,Sub_Platform_Name asc,CK_Package_Type asc";
}
