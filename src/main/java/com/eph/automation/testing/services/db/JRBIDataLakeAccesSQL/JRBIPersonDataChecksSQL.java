...
public class JRBIPersonDataChecksSQL {
    ...
    public static String GET_PERSON_RECORDS_FULL_LOAD =
           /* "select epr as EPR" +
                    ...
                    "LEFT JOIN "+GetSDBooksDLDBUser.getProductDatabase()+".eph_identifier_cross_reference_v cr1 ON ((((j.issn = cr1.identifier) AND (cr1.identifier_type = 'ISSN')) AND (cr1.record_level = 'n')) AND (cr1.record_level = 'Work')))\n" +
                     "LEFT JOIN "+GetSDBooksDLDBUser.getProductDatabase()+".eph_identifier_cross_reference_v cr2 ON (((j.journal_number = cr2.identifier) AND (cr2.identifier_type = 'ELSEVIER JOURNAL NUMBER')) AND (cr2.record_level = 'Work'))))\n" +
                    "where EPR in ('%s')";*/
    ...
    public static String GET_DELTA_PERSON_HISTORY_RECORDS =
            "select epr as EPR" +
                    ...
                    //"delta_ts like \'%%"+SDDataLakeCountChecksSQL.previousDate()+"%%\'";
