...
/**
 * Created by RAVIVARMANS on 11/25/2018. //updated Dinesh on 19/04/2021 replaced the connection with
 * SecretManager instead of Load properties
 */
public class DBManager {
  ...
  public static List getDBResultAsBeanList(String sql, Class klass, String dbEndPoint) {
    ...
    // dbProps.setProperty("jdbcUrl",
    // LoadProperties.getDBConnection(getDatabaseEnvironmentKey(dbEndPoint)));
  ...
  public static Connection getPostgresConnection(String dbEndPoint) {
    Properties dbProps = new Properties();
    dbProps.setProperty(
        "jdbcUrl", LoadProperties.getDBConnection(getDatabaseEnvironmentKey(dbEndPoint)));
    if (connection == null) {
      try {
  ...
  public static String getDatabaseEnvironmentKey(String givenDBEndPoint) {
    ...
    if (EnumConstants.ENVIRONMENTS.CI.name().equalsIgnoreCase(TestContext.getValues().environment)
        && TestContext.getValues().targetDB) {
      // Get CI
    } else if (EnumConstants.ENVIRONMENTS
            .SIT
            .name()
            .equalsIgnoreCase(TestContext.getValues().environment)
        && !TestContext.getValues().targetDB) {
      // Get SIT
    } else if (EnumConstants.ENVIRONMENTS
            .UAT
    ...
    try {
      ...
      String user =
          StringUtils.substring(
                  URL, StringUtils.indexOf(URL, "?") + 1, StringUtils.indexOf(URL, "&"))
              .replace("user=", "")
              .trim();
      String pwd =
          StringUtils.substring(URL, StringUtils.indexOf(URL, "&") + 1)
              .replace("password=", "")
              .trim();
